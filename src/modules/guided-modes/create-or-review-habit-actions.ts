...
import {
  buildCreateOrReviewHabitMetadata,
  buildCreateOrReviewHabitSystemInstructions,
  buildLocalCreateOrReviewHabitReply,
  buildLocalCreateOrReviewHabitSummary,
  createInitialCreateOrReviewHabitProgress,
  createOrReviewHabitModeCode,
  parseCreateOrReviewHabitProgress,
  recordCreateOrReviewHabitAnswer,
  type CreateOrReviewHabitProgress,
...
export async function getCreateOrReviewHabitView(
  userId: string,
  agentId: string,
) {
  const mode = await getActiveCreateOrReviewHabitMode();
  ...
  const session =
    (await getLatestCreateOrReviewHabitSession(
      userId,
      agentId,
      mode.id,
      "active",
  ...
  const activeSession =
    session?.status === "active"
      ? session
      : (session ??
        (await createCreateOrReviewHabitSession(userId, agentId, mode)));
  const progress = parseCreateOrReviewHabitProgress(activeSession.metadata);
  const sessionMessages = await getCreateOrReviewHabitMessages(
    userId,
    activeSession.id,
  );
...
export async function sendCreateOrReviewHabitMessage(
  ...
  if (!mode) {
    return {
      error:
        "El modo Crear o revisar un hábito no está disponible ahora mismo.",
    };
  }

  const prepared = await db.transaction(async (tx) => {
    const session = await getOrCreateActiveCreateOrReviewHabitSession(
      tx,
      user.id,
      primaryAgent.id,
      mode,
    );
    const progress = parseCreateOrReviewHabitProgress(session.metadata);
    ...
    const reservation = await reserveCreditsForSession({
      ...
      idempotencyKey: `guided-habit-reservation:${session.id}`,
    });
  ...
  const nextProgress = prepared.safetyAssessment.shouldInterrupt
    ? prepared.progress
    : recordCreateOrReviewHabitAnswer(prepared.progress, parsed.data.content);
  ...
  await persistCreateOrReviewHabitTurn({
    userId: user.id,
  ...
  revalidatePath("/modos/habito");
  redirect("/modos/habito");
}

export async function startNewCreateOrReviewHabitSession() {
  const user = await getCurrentUser();
  ...
  await createCreateOrReviewHabitSession(user.id, primaryAgent.id, mode);
...
async function getActiveCreateOrReviewHabitMode(): Promise<GuidedMode | null> {
  const [mode] = await db
    .select({
      id: guidedModes.id,
    ...
    .where(
      and(
        eq(guidedModes.code, createOrReviewHabitModeCode),
...
async function getLatestCreateOrReviewHabitSession(
  userId: string,
  agentId: string,
  guidedModeId: string,
  status: string,
...
async function createCreateOrReviewHabitSession(
  userId: string,
  agentId: string,
  mode: GuidedMode,
) {
  return db.transaction(async (tx) =>
    getOrCreateActiveCreateOrReviewHabitSession(tx, userId, agentId, mode),
  );
}

async function getOrCreateActiveCreateOrReviewHabitSession(
  tx: Transaction,
  userId: string,
  agentId: string,
  mode: GuidedMode,
  ...
  const [conversation] = await tx
    ...
    .values({
      userId,
      agentId,
      title: "Crear o revisar un hábito",
  ...
  if (!conversation) {
    throw new Error("Guided habit conversation could not be created.");
  }
  ...
  const [session] = await tx
    ...
    .values({
      ...
      pricingRuleVersion: "mvp-guided-habit-v1",
      metadata: buildCreateOrReviewHabitMetadata(
        null,
        createInitialCreateOrReviewHabitProgress(),
      ),
    })
  ...
  if (!session?.conversationId) {
    throw new Error("Guided habit session could not be created.");
  }
...
async function getCreateOrReviewHabitMessages(
  userId: string,
  sessionId: string,
) {
  return db
...
async function buildGuidedAssistantReply(input: {
  ...
  const reply = await generateText({
    ...
    systemInstructions: buildCreateOrReviewHabitSystemInstructions({
  ...
  if (reply.finishReason === "error") {
    return {
      content: buildLocalCreateOrReviewHabitReply(input.progress),
      provider: "local",
      model: "guided-habit-flow-v1",
      inputTokens: null,
      outputTokens: null,
      latencyMs: 0,
      correlationId,
...
async function persistCreateOrReviewHabitTurn(input: {
  userId: string;
  agentId: string;
  ...
  await db.transaction(async (tx) => {
    ...
    if (!userMessage) {
      throw new Error("Guided habit user message could not be created.");
    }
    ...
    if (!assistantMessage) {
      throw new Error("Guided habit assistant message could not be created.");
    }
    ...
    if (input.progress.completed) {
      await completeCreateOrReviewHabitSession({
        tx,
        userId: input.userId,
        sessionId: input.session.id,
        conversationId: input.session.conversationId,
...
async function completeCreateOrReviewHabitSession(input: {
  tx: Transaction;
  userId: string;
  sessionId: string;
  conversationId: string;
  ...
  await input.tx
    ...
    .values({
      ...
      summary:
        ...
        buildLocalCreateOrReviewHabitSummary(input.progress.answers),
      mainTopic: input.progress.answers[1]?.answer.slice(0, 160) ?? null,
      keyPoints: input.progress.answers.map(
        (answer) => `${answer.stageTitle}: ${answer.answer.slice(0, 180)}`,
      ),
      decisions: input.progress.answers
        .filter((answer) => answer.stageId === "minimal_action")
        .map((answer) => answer.answer.slice(0, 220)),
      nextSteps: input.progress.answers
        .filter((answer) => answer.stageId === "review_moment")
        .map((answer) => answer.answer.slice(0, 220)),
      memoryCandidates: [],
      safetySummary: null,
      provider: "local",
      model: "guided-habit-summary-v1",
      status: "completed",
    })
    .onConflictDoNothing({
