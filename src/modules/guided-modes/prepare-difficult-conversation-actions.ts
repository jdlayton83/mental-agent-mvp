...
import {
  buildLocalPrepareDifficultConversationReply,
  buildLocalPrepareDifficultConversationSummary,
  buildPrepareDifficultConversationMetadata,
  buildPrepareDifficultConversationSystemInstructions,
  createInitialPrepareDifficultConversationProgress,
  parsePrepareDifficultConversationProgress,
  prepareDifficultConversationModeCode,
  recordPrepareDifficultConversationAnswer,
  type PrepareDifficultConversationProgress,
...
export async function getPrepareDifficultConversationView(
  userId: string,
  agentId: string,
) {
  const mode = await getActivePrepareDifficultConversationMode();
  ...
  const session =
    (await getLatestPrepareDifficultConversationSession(
      userId,
      agentId,
      mode.id,
      "active",
  ...
  const activeSession =
    session?.status === "active"
      ? session
      : (session ??
        (await createPrepareDifficultConversationSession(
          userId,
          agentId,
          mode,
        )));
  const progress = parsePrepareDifficultConversationProgress(
    activeSession.metadata,
  );
  const sessionMessages = await getPrepareDifficultConversationMessages(
    userId,
    activeSession.id,
  );
...
export async function sendPrepareDifficultConversationMessage(
  ...
  if (!mode) {
    return {
      error:
        "El modo Preparar una conversación difícil no está disponible ahora mismo.",
    };
  }

  const prepared = await db.transaction(async (tx) => {
    const session = await getOrCreateActivePrepareDifficultConversationSession(
      tx,
      user.id,
      primaryAgent.id,
      mode,
    ...
    const reservation = await reserveCreditsForSession({
      ...
      idempotencyKey: `guided-difficult-conversation-reservation:${session.id}`,
    });
  ...
  await persistPrepareDifficultConversationTurn({
    userId: user.id,
  ...
  revalidatePath("/modos/conversacion-dificil");
  redirect("/modos/conversacion-dificil");
}

export async function startNewPrepareDifficultConversationSession() {
  const user = await getCurrentUser();
...
async function getActivePrepareDifficultConversationMode(): Promise<GuidedMode | null> {
  const [mode] = await db
    .select({
      id: guidedModes.id,
    ...
    .where(
      and(
        eq(guidedModes.code, prepareDifficultConversationModeCode),
...
async function getLatestPrepareDifficultConversationSession(
  userId: string,
  agentId: string,
  guidedModeId: string,
  status: string,
...
async function createPrepareDifficultConversationSession(
  userId: string,
  agentId: string,
  mode: GuidedMode,
) {
  return db.transaction(async (tx) =>
    getOrCreateActivePrepareDifficultConversationSession(
      tx,
      userId,
      agentId,
      mode,
...
async function getOrCreateActivePrepareDifficultConversationSession(
  tx: Transaction,
  userId: string,
  agentId: string,
  mode: GuidedMode,
  ...
  if (!conversation) {
    throw new Error("Guided difficult conversation could not be created.");
  }
  ...
  const [session] = await tx
    ...
    .values({
      ...
      pricingRuleVersion: "mvp-guided-difficult-conversation-v1",
      metadata: buildPrepareDifficultConversationMetadata(
        null,
        createInitialPrepareDifficultConversationProgress(),
      ),
    })
  ...
  if (!session?.conversationId) {
    throw new Error(
      "Guided difficult conversation session could not be created.",
    );
  }
...
async function getPrepareDifficultConversationMessages(
  userId: string,
  sessionId: string,
) {
  return db
...
async function buildGuidedAssistantReply(input: {
  ...
  const reply = await generateText({
    ...
    systemInstructions: buildPrepareDifficultConversationSystemInstructions({
  ...
  if (reply.finishReason === "error") {
    return {
      content: buildLocalPrepareDifficultConversationReply(input.progress),
      provider: "local",
      model: "guided-difficult-conversation-flow-v1",
      inputTokens: null,
      outputTokens: null,
      latencyMs: 0,
      correlationId,
...
async function persistPrepareDifficultConversationTurn(input: {
  userId: string;
  agentId: string;
  ...
  await db.transaction(async (tx) => {
    ...
    if (!userMessage) {
      throw new Error(
        "Guided difficult conversation user message could not be created.",
      );
    }
    ...
    if (!assistantMessage) {
      throw new Error(
        "Guided difficult conversation assistant message could not be created.",
      );
    }
    ...
    if (input.progress.completed) {
      await completePrepareDifficultConversationSession({
        tx,
        userId: input.userId,
        sessionId: input.session.id,
        conversationId: input.session.conversationId,
...
async function completePrepareDifficultConversationSession(input: {
  tx: Transaction;
  userId: string;
  sessionId: string;
  conversationId: string;
  ...
  await input.tx
    ...
    .values({
      ...
      summary:
        ...
        buildLocalPrepareDifficultConversationSummary(input.progress.answers),
      ...
      decisions: input.progress.answers
        .filter((answer) => answer.stageId === "core_message")
        .map((answer) => answer.answer.slice(0, 220)),
      nextSteps: input.progress.answers
        .filter((answer) => answer.stageId === "boundary_or_request")
        .map((answer) => answer.answer.slice(0, 220)),
      memoryCandidates: [],
      safetySummary: null,
      provider: "local",
      model: "guided-difficult-conversation-summary-v1",
      status: "completed",
    })
    .onConflictDoNothing({
